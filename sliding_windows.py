...
def fit_polynomial(binary_warped):
    ...
    def main(binary_warped):
        ...
        if len(window_centroids) > 0:
            ...
            output = cv2.addWeighted(warpage, .7, template, 1, 0.0) # overlay the original road image with window results
    ...
    def radius_of_curvature(binary_warped,window_centroids):
        ploty = np.linspace(0, binary_warped.shape[0], num=nwindows) # to cover same y-range as image
    ...
    def convolve(binary_warped):
        ...
        bottom_image = image[int(image.shape[0]*(3/4)):]
        
        # divide images into left and right
        l_img = bottom_image[:,:int(image.shape[1]/2)]
        r_img = bottom_image[:,int(image.shape[1]/2):]
        # Sum 
        ...
        r_center = np.argmax(r_convolve)+ int(image.shape[1]/2)
        ...
        for level in range(1,(int)(image.shape[0]/window_height)):
            ...
            image_layer = image[int(image.shape[0]-(level+1)*window_height):int(image.shape[0]-level*window_height),:]
